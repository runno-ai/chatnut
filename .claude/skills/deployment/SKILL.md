...
Use when monitoring or triggering the agents-chat-mcp CD pipeline. Checks workflow
...
```bash
RUN_ID=$(gh run list --workflow=cd.yml --repo runno-ai/agents-chat-mcp --limit 1 \
  --json databaseId --jq '.[0].databaseId')
gh run view "$RUN_ID" --repo runno-ai/agents-chat-mcp --log
```
...
gh workflow run cd.yml --repo runno-ai/agents-chat-mcp \
  --ref "$BRANCH" \
  --field branch="$BRANCH"
